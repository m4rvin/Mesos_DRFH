...
./mesos-agent.sh --master=127.0.0.1:5050 --work_dir=/tmp/mesos/agent9 --no-systemd_enable_support --log_dir=/tmp/mesosLog/agent9 --resources="cpus:64;mem:262144" --advertise_port=5059 --port=5059 &
pid=$!
echo $pid
echo $pid >> "$CLUSTER_NODES_PIDS_FILEPATH"

./mesos-agent.sh --master=127.0.0.1:5050 --work_dir=/tmp/mesos/agent10 --no-systemd_enable_support --log_dir=/tmp/mesosLog/agent10 --resources="cpus:64;mem:262144" --advertise_port=5060 --port=5060 &
pid=$!
echo $pid
echo $pid >> "$CLUSTER_NODES_PIDS_FILEPATH"
